 ...
 *  RandomForest randomly selects sub samples with size = bR * sample size from the sample (with replacement) and uses the fS number of sub features to build the trees, and classify the sample by voting the major results of the trees
 *   
        ...
        extends ClassifierReal (x, y, fn, k , cn) with Error
{
    ...
    {   if (ver == true) println(s"predict for $z:")
        var result = new VectorI (k)
        for (i <- 0 until nF) {
          result(forest(i).classify (z)._1) += 1
          if (ver == true) println(s"for tree$i, predict class=${cn(forest(i).classify (z)._1)}")}
...
{
    val x = new MatrixD ((11,11), 8.1,0.27,0.41,1.45,0.033,11,63,0.9908,2.99,0.56,12,
    ...
    y.-=(3)  
    val numClasses = 7
    val fn = Array ("fixed acidity","volatile acidity","citric acid","residual sugar","chlorides","free sulfur dioxide","total sulfur dioxide","density","pH","sulphates","alcohol")        // feature names

    val cn = Array("Level3", "Level4", "Level5", "Level6", "Level7", "Level8", "Level9") //class names
    val rF = new RandomForest (x, y, nF = 5, bR = 0.7, fS = 7, k = numClasses, s = 223, fn = fn, cn = cn, ver = true)
    rF.train()
